...
export async function checkVoiceRateLimit(session: Session, minutesRequested: number = 1): Promise<RateLimitResult> {
  ...
  if (!rateLimit) {
    // Create new rate limit with separate periods for messages and voice
    const messagePeriodEnd = new Date(now.getTime() + limits.messagePeriodHours * 60 * 60 * 1000);
    const voicePeriodEnd = new Date(now.getTime() + limits.voicePeriodHours * 60 * 60 * 1000);
    rateLimit = await storage.createRateLimit({
      sessionId: session.id,
      periodStart: now,
      periodEnd: messagePeriodEnd,
      messagesUsed: 0,
      voiceMinutesUsed: 0,
      voicePeriodStart: now,
      voicePeriodEnd,
    });
  } else if (!rateLimit.voicePeriodEnd || now > rateLimit.voicePeriodEnd) {
    // Voice period expired or not set - reset voice usage and create new period
    const voicePeriodEnd = new Date(now.getTime() + limits.voicePeriodHours * 60 * 60 * 1000);
    await storage.updateRateLimit(rateLimit.id, {
      voiceMinutesUsed: 0,
      voicePeriodStart: now,
    ...
    rateLimit = await storage.getRateLimit(session.id);
    if (!rateLimit) {
  ...
  return {
    ...
    resetTime: rateLimit.voicePeriodEnd || rateLimit.periodEnd,
  };
}
