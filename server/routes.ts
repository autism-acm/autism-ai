...
import { generateConversationTitle } from "./utils/summaryGenerator";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Get conversation summaries (AUtism Summary - AI-generated topic index)
  app.get("/api/conversations/summaries", async (req, res) => {
    try {
      const session = req.session!;
      const conversations = await storage.getConversationsBySession(session.id);
      
      // Sort by createdAt to maintain chronological order, then number them
      const sortedConversations = [...conversations].sort((a, b) => 
        new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime()
      );
      
      // Return conversations with their titles as topic summaries
      const summaries = sortedConversations
      ...
      res.status(500).json({ error: "Failed to get conversation summaries" });
    }
  });

  // Get topic index for a specific conversation
  app.get("/api/conversations/:id/topics", async (req, res) => {
    try {
      const { id } = req.params;
      const session = req.session!;
      ...
      // Generate topic index from messages
      const { generateConversationTopicIndex } = await import('./utils/summaryGenerator');
      const topics = await generateConversationTopicIndex(
      ...
      res.status(500).json({ error: "Failed to get conversation topics" });
    }
  });
  ...
  app.post("/api/messages", async (req, res) => {
    try {
      ...
      try {
        ...
        if (requestImage) {
          // For image requests, use ONLY the enhanced prompt (DO NOT include Memory Bank to prevent data leakage)
          ...
          // Build context with Memory Bank for text responses only
          let fullPrompt = enhancedPrompt;
          if (session.memoryBank && session.memoryBank.trim()) {
            fullPrompt = `[MEMORY BANK - Remember this about the user]\n${session.memoryBank}\n\n[USER MESSAGE]\n${enhancedPrompt}`;
          }
        ...
        // Auto-generate conversation title ONLY if this is the first message in a new conversation
        // Check if conversation has exactly 2 messages now (1 user + 1 assistant)
        const messageCount = await storage.getMessagesByConversation(conversation.id);
        if (messageCount.length === 2 && !conversation.lastSummaryAt) {
          try {
