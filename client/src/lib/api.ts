...
export const api = {
  ...
  conversations: {
    ...
    getSummaries: async (): Promise<ConversationSummary[]> => {
      const response = await fetch('/api/conversations/summaries', { credentials: 'include' });
      if (!response.ok) throw new Error('Failed to get conversation summaries');
      return response.json();
    },

    getTopics: async (conversationId: string): Promise<string[]> => {
      const response = await fetch(`/api/conversations/${conversationId}/topics`, {
        credentials: 'include',
      });
      if (!response.ok) throw new Error('Failed to get conversation topics');
      return response.json();
    },
