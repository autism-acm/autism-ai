...
import { Send, Phone, PhoneOff, Loader2, Sparkles, Brain, Zap, Paperclip, X, FileText, Link2 } from 'lucide-react';
...
export default function ChatInterface({ 
  ...
  // Handle URL intent parameter (?intent="TEXT")
  useEffect(() => {
    const urlParams = new URLSearchParams(window.location.search);
    const intentText = urlParams.get('intent');
    
    if (intentText) {
      // Set the input value to the intent text
      setInput(intentText);
      
      // Remove the intent parameter from the URL without reloading
  ...
  const sendMessageMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      if (isNewConversation) {
        ...
        queryClient.invalidateQueries({ queryKey: ['conversationSummaries'] });
      }
  ...
  const toggleRecording = async () => {
    ...
    if (isVoiceConnecting) {
      console.log('[Voice] Already connecting, ignoring click');
      return;
    }
  ...
  const startVoiceRecording = async () => {
    // Set connecting guard IMMEDIATELY before any async operations
    setIsVoiceConnecting(true);
    
    try {
      // Check if WebSocket is already open (continuous conversation)
      if (voiceWebSocketRef.current && voiceWebSocketRef.current.readyState === WebSocket.OPEN) {
        // Reuse existing WebSocket, just restart MediaRecorder
        console.log('[Voice] Reusing existing WebSocket for continuous conversation');
        
        // Clean up old media tracks before creating new ones (prevent leaks!)
        cleanupMediaTracks();
        ...
        toast({
          ...
          description: "Speak now, AI is listening...",
        });
        return;
      }
      ...
      if (!activeConversationId) {
        try {
          ...
          console.log('[Voice] Created new conversation:', newConversation.id);
        } catch (error) {
          console.error('[Voice] Failed to create conversation:', error);
          setIsVoiceConnecting(false); // Clear on error
          toast({
            title: "Error",
      ...
      // Request microphone permission for new session
      const stream = await navigator.mediaDevices.getUserMedia({ audio: true });

      // Create WebSocket connection with personality and conversationId as query params
      // Use correct protocol (wss:// for HTTPS, ws:// for HTTP)
      const protocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:';
      const wsUrl = `${protocol}//${window.location.host}/api/voice-stream?personality=${encodeURIComponent(selectedCharacter)}&conversationId=${activeConversationId}`;
      const ws = new WebSocket(wsUrl);
      voiceWebSocketRef.current = ws;

      ws.onopen = () => {
        console.log('[Voice] WebSocket connected');
        setIsVoiceConnecting(false); // Connection successful
        ...
        // Reset audio chunks buffer for new recording
        audioChunksRef.current = [];

        mediaRecorder.ondataavailable = (event) => {
          if (event.data.size > 0) {
            // Accumulate chunks in ref instead of sending immediately
            audioChunksRef.current.push(event.data);
          }
        };

        // Start recording with chunks every 100ms (for buffering)
        mediaRecorder.start(100);
        setIsRecording(true);
      ...
      ws.onmessage = (event) => {
        ...
        if (data.type === 'transcript') {
          ...
          // Show processing status (optional)
          console.log('[Voice] Processing:', data.message);
        } else if (data.type === 'error') {
          toast({
            title: "Voice Error",
            description: data.message || "An error occurred during voice streaming",
            variant: "destructive",
          });
        }
      };
      ...
      ws.onerror = (error) => {
        ...
        setIsVoiceConnecting(false); // Clear connecting state on error
        toast({
      ...
      ws.onclose = () => {
        ...
        setIsVoiceConnecting(false); // Clear connecting state on close
        // Only cleanup if recording is still active (prevent recursive loop)
        if (isRecording) {
          cleanupVoiceSession();
        }
      };
      ...
      setIsVoiceConnecting(false); // Clear connecting state on exception
      toast({
  ...
  const cleanupMediaTracks = () => {
    // Stop all media tracks to prevent leaks
    if (mediaRecorderRef.current && mediaRecorderRef.current.stream) {
      mediaRecorderRef.current.stream.getTracks().forEach(track => {
        track.stop();
        console.log('[Voice] Stopped media track');
      });
    }
  };

  const cleanupVoiceSession = () => {
    // Stop media recorder and its tracks
  ...
  const stopVoiceRecording = () => {
    // Stop MediaRecorder first to flush any remaining chunks
    if (mediaRecorderRef.current && mediaRecorderRef.current.state !== 'inactive') {
      mediaRecorderRef.current.stop();
    }

    // Send buffered audio chunks to backend
    const sendBufferedAudio = () => {
      // Reset buffer first
      const chunks = [...audioChunksRef.current];
      audioChunksRef.current = [];

      if (chunks.length > 0 && voiceWebSocketRef.current && voiceWebSocketRef.current.readyState === WebSocket.OPEN) {
        const audioBlob = new Blob(chunks, { type: 'audio/webm;codecs=opus' });
        const reader = new FileReader();
        reader.onloadend = () => {
          const base64Audio = reader.result?.toString().split(',')[1];
          if (base64Audio && voiceWebSocketRef.current && voiceWebSocketRef.current.readyState === WebSocket.OPEN) {
            // Send audio
            voiceWebSocketRef.current.send(JSON.stringify({
              type: 'audio_input',
              audio: base64Audio,
            }));
            console.log('[Voice] Sent complete audio utterance, waiting for response...');

            // Keep WebSocket open to receive transcript, ai_response, and audio_output
            // Don't close yet! Let the backend send responses.
            // Only mark as not recording so user can speak again
            setIsRecording(false);
            ...
            // Failed to convert audio or WebSocket closed
            toast({
              title: "Error",
              description: "Failed to send voice recording",
              variant: "destructive",
            });
            cleanupVoiceSession();
            setIsRecording(false);
        ...
        // No audio chunks to send
        if (chunks.length === 0) {
          toast({
            title: "No Audio",
            description: "No voice data recorded",
            variant: "destructive",
          });
        }
        
        // Just close and cleanup
        if (voiceWebSocketRef.current && voiceWebSocketRef.current.readyState === WebSocket.OPEN) {
          voiceWebSocketRef.current.send(JSON.stringify({
            type: 'stop',
          }));
    ...
    // Give MediaRecorder time to flush final chunks before sending
    setTimeout(() => {
      sendBufferedAudio();
    }, 150);
          ...
          <div className="bg-[#2a2a2a] border-[#404040] border shadow-lg p-3 w-full box-border max-w-full" style={{ 
            ...
            <div className="flex items-start gap-2 max-w-full">
              ...
              {!isMobile && (
                ...
                >
                  {isRecording ? <PhoneOff className="h-3.5 w-3.5 text-white" /> : <Phone className="h-3.5 w-3.5" />}
                </Button>
              )}
