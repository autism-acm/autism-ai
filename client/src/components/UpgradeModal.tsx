      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
          <TierCard 
            ...
            isCurrentTier={session?.tier === 'Free Trial'}
            walletAddress={session?.walletAddress}
            tokenBalance={session?.tokenBalance}
          />
          <TierCard 
            name="Electrum"
            requirement="Hold 100k $AU"
            requiredTokens={100000}
            messagesPerHour={20}
            voiceHoursPerDay={1}
            isUpgrade={true}
            isCurrentTier={session?.tier === 'Electrum'}
          ...
          <TierCard 
            name="Pro"
            requirement="Hold 200k $AU"
            requiredTokens={200000}
            messagesPerHour={40}
            voiceHoursPerDay={2}
            isUpgrade={true}
            isCurrentTier={session?.tier === 'Pro'}
            onPurchase={handlePurchaseTokens}
            walletAddress={session?.walletAddress}
            tokenBalance={session?.tokenBalance}
          />
          <TierCard 
            name="Gold"
            requirement="Hold 300k $AU"
            requiredTokens={300000}
            messagesPerHour={50}
            voiceHoursPerDay={4}
            isUpgrade={true}
            isCurrentTier={session?.tier === 'Gold'}
            onPurchase={handlePurchaseTokens}
            walletAddress={session?.walletAddress}
            tokenBalance={session?.tokenBalance}
          />
